...
| # | Change | Where |
| --- | --- | --- |
| 1 | **Run it now.** The API request under a result and in the docs sends itself with the person's real key and shows the response. First 2xx per browser fires `api_first_call_succeeded` (sources `in_app_test`, `api_docs_test`); every run fires `api_test_call_started` / `_succeeded` / `_failed`. The worker-side half, for calls made from outside the app, is specified in `workers/api-first-call/README.md`. | `app.html` (`runNextStepApiTest`), `api-documentation.html` |
