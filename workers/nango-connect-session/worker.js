/**
 * nango-connect-session — HubSpot CRM sync.
 *
 * Pulled out of the Cloudflare dashboard and committed on 2026-08-25. Read
 * CRM-SETUP.md for the route contract and the deployment checklist; read
 * DISCONNECT-AND-GATE-FIX.md in this directory for what changed here and why.
 *
 * Connect a HubSpot account, and on a weekly cron this worker finds contacts
 * missing whichever fields the user chose to track — email, LinkedIn URL,
 * and/or phone — looks them up via LinkFinder's own enrichment API, and writes
 * them back to HubSpot. ONLY into empty fields, never overwriting something a
 * human already entered ("no override").
 *
 * WHY THIS EXISTS: PostHog data showed 41% of stated cancellation reasons are
 * "notUsing" — people churn because they stopped opening the product, not
 * because of price or a missing feature. A live CRM connection creates value in
 * the background, without requiring the user to remember the tool exists.
 *
 * ARCHITECTURE: uses Nango (nango.dev) purely for OAuth token storage + a
 * proxied HubSpot API — NOT Nango's own hosted Functions/Syncs runtime (that
 * pauses every 2 weeks on the free tier). Everything here is plain REST calls
 * to api.nango.dev via fetch(), run on this worker's own cron.
 *
 * Enrichment calls go through a SERVICE BINDING (env.ENRICH_SERVICE), not a
 * plain fetch() to the public workers.dev URL. Cloudflare blocks a Worker from
 * fetching another Worker in the SAME account over its public *.workers.dev
 * hostname (error 1042, a loop-prevention rule). A Service Binding routes over
 * Cloudflare's internal network instead.
 */
...
// Delete the connection AT NANGO, not just our KV record.
//
// This used to not exist, and /disconnect only did a KV delete. Two
// consequences, both bad: Nango bills per connection for as long as the
// connection exists, so every user who ever pressed Disconnect stayed
// billable forever while being invisible to this worker; and the OAuth grant
// stayed live on the customer's HubSpot portal after they had told us to let
// go of it. Neither is acceptable, and the second one is the more serious.
//
// Nango has moved this endpoint: /connections/:id is current, /connection/:id
// is the older documented one and still functions. We cannot reach
// docs.nango.dev from CI to confirm which this account's environment serves,
// so try the current path first and fall back. Both outcomes are logged, so
// the first real disconnect tells you which one answered — once you know,
// delete the loser.
async function nangoDeleteConnection(env, connectionId) {
  const q = `?provider_config_key=${encodeURIComponent(HUBSPOT_INTEGRATION_ID)}`;
  const paths = [`/connections/${encodeURIComponent(connectionId)}${q}`,
                 `/connection/${encodeURIComponent(connectionId)}${q}`];

  for (const path of paths) {
    try {
      const r = await fetch(`${NANGO_API_BASE}${path}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${env.NANGO_SECRET_KEY}` },
      });
      if (r.ok) {
        console.log(`[crm-sync] deleted Nango connection ${connectionId} via ${path}`);
        return { ok: true, path };
      }
      // 404 means "wrong path shape" or "already gone" — either way, try the
      // other spelling before giving up.
      console.log(`[crm-sync] Nango DELETE ${path} -> ${r.status} ${await r.text().catch(() => '')}`.slice(0, 300));
    } catch (e) {
      console.error(`[crm-sync] Nango DELETE ${path} threw:`, e && e.message);
    }
  }
  return { ok: false };
}
...
//
// CALL IT AS isSubscriber(env, token). Two call sites used to pass only the
// token. That is not a type error in JS: `env` silently became the token
// string, `token` became undefined, the POST went out with no token at all,
// and the helper's own fail-closed catch returned false. Result: /push-contacts
// answered 403 for everyone, and the weekly cron skipped every single
// connection as "not a subscriber". The whole feature was dead and looked
...
// Disconnect now actually releases the connection at Nango before dropping
// our KV record, and drops the KV record either way. Order matters: we need
// the connectionId, which only lives in KV, to make the Nango call.
async function handleDisconnect(request, env, origin) {
  ...
  if (raw) {
    try {
      const { connectionId } = JSON.parse(raw);
      if (connectionId) released = (await nangoDeleteConnection(env, connectionId)).ok;
    } catch (e) {
      console.error('[crm-sync] disconnect could not read stored connection:', e && e.message);
    }
  }

  // Always forget it locally. If the Nango delete failed we have logged it and
  // reported `released:false`; leaving a stale KV row on top of that would mean
  // the weekly cron keeps trying to sync an account the user has disowned.
  await env.CRM_CONNECTIONS.delete(`conn:${token}`);
  return json({ ok: true, released }, 200, origin);
}

// Nango webhook — fires when a connection is created (and other lifecycle
// events). Best-effort payload parsing.
...
export default {
  async fetch(request, env) {
    ...
    if (!env.CRM_CONNECTIONS) {
      return json({ error: 'CRM_CONNECTIONS KV namespace not bound - see CRM-SETUP.md' }, 500, origin);
    }
    if (!env.NANGO_SECRET_KEY) {
      return json({ error: 'NANGO_SECRET_KEY not set - see CRM-SETUP.md' }, 500, origin);
    }
    ...
    if (url.pathname === '/sync-now' || url.pathname === '/test-sync') {
      if (!env.ENRICH_SERVICE) {
        return json({ error: 'ENRICH_SERVICE binding not set — add a Service binding to the linkfinderapp worker (see CRM-SETUP.md). Without it every lookup fails with Cloudflare error 1042.' }, 500, origin);
      }
