/**
 * crm-sync worker  (deployed Cloudflare Worker name: `nango-connect-session`)
 *
 ...
 * per weekly run is user-configurable too (clamped server-side, see
 * HARD_MAX_PER_RUN).
 *
 ...
 * 1. The deployed Worker must be named EXACTLY `nango-connect-session`
 *    — account.html and crm-sync.html have this worker's expected URL
 *    hardcoded as `https://nango-connect-session.hamoureliasse.workers.dev/`.
...
//
// NOTE ON THE SIGNATURE: this takes (env, token), in that order. Calling it
// with a bare token silently degrades rather than throwing — `env` becomes
// the token string, `env.SUBSCRIBER_SERVICE` is undefined so it falls back
// to global fetch, and `token` is undefined so JSON.stringify drops the key
// entirely and posts `{"trigger":"crm_sync_gate"}` with no token at all.
// upgrade-intent then answers "not a subscriber" and the gate fails closed
// for everyone, forever, with no error anywhere. Two call sites did exactly
// that until 24 Aug 2026 — see the git history for this file.
